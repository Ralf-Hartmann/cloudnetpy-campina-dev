...
class Parsivel(Disdrometer):
    ...
    def _screen_time(self, expected_date: datetime.date | None = None) -> None:
        ...
        # print(f"Raw time data: {self.raw_data['time']}")
        # print(f"Expected date: {self.date}")
        ...
        for key in self.raw_data:
            if len(self.raw_data[key]) == len(valid_mask):
                #print(f"Filtering '{key}'...")
                self.raw_data[key] = self.raw_data[key][valid_mask]
                #print(f"Column '{key}' after mask: {len(self.raw_data[key])}")
            else:
                print(f"Skipping '{key}' due to dimension mismatch: {len(self.raw_data[key])} vs {len(valid_mask)}")
        
        # Raise error if no data remains
...
'''Removed read in functions for specified file formats with known headers!'''
...
def _read_parsivel(
    ...
    for key, value in combined_data.items():
        ...
        #print(f"Key {key}, NaN count: {np.isnan(array).sum() if np.issubdtype(array.dtype, np.floating) else 'N/A'}")
        #print(f"{key} Min: {array.min()}, Max: {array.max()}")

        #print(f"Raw {key}: {array}")
        mask = [np.full(array.shape[1:], x is None) for x in value]
    
        #print(f"\nKey: {key}")
        #print(f"Array dtype: {ndtype}")
        #print(f"Array sample: {array[:10]}")  # Print first 10 for inspection
        #print(f"Mask sample: {mask[:10]}")
        #print(f"Fill value to be applied: {EMPTY_VALUES.get(PARSERS.get(key, _parse_float))}")

        #print(f"Key {key}, Array: {array}")
        #print(f"Array dtype for {key}: {array.dtype}")
        #print(f"Mask for {key}: {mask}")
        #print(f"Mask Shape: {mask.shape}, Array Shape: {array.shape}")
        
        result[key] = ma.array(array, mask=mask)
        #print(f"{key}: {array[:10]}")
        #result[key] = array
        #print(f"After Masking {key}: {result[key]}")
